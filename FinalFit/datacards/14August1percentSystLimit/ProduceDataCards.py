...
class DatasetInfo :
    ...
    def MakeANewWS(self , ct, cv , newws ):
        ...
        if self.Sample == "thw" :
            factor = 1.
        elif self.Sample == "tth" :
            factor = 1.01 # for ggH
            additive = 0.112468 #remaining vh events @ SM
            additive *= kappa.GetXSecBR( "vh" , ct , cv )
            
        self.new_norm = RooRealVar( self.PDF + "_norm"  , self.PDF + "_norm"  , factor*self.norm.getVal()+additive ) # , factor*self.Low*self.norm.getVal() , factor*self.High*self.norm.getVal() )
...
class BinDatacard :
    def __init__(self, BinName  , EffPlotName = None , bkgRate = 1. ):
        ...
        #self.HiggsSamples["vh"]  = DatasetInfo("vh" , "../../signals/14August/out_vh_syst.root", "cms_hgg_13TeV", "RVvh_mh125" ,  "2dPlots.root" , EffPlotName, 0.99999 , 1.00001 , self.Cut )
...
Preselection = BinDatacard( "THQLeptonicTag", "Preselection" , bkgRate=1 )
AllBins = {}
AllBins["THQLeptonic"] = {"THQ":BinDatacard( "THQLeptonicTHQTag" , bkgRate=3./35 ) ,
                          "TTH":BinDatacard( "THQLeptonicTTHTag" , bkgRate=32./35) }
...
for kvkt in AllCtOverCVs:
    ...
    fRun.write("cd /eos/user/h/hbakhshi/Personal/Projects/tHq/Analyzer/FinalFit/datacards/14August1percentSystLimit/%s\n" % (dirname) )
