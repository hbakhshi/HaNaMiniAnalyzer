...
class BinDatacard :
    def __init__(self, BinName  , EffPlotName = None , bkgRate = 1. ):
        ...
        lenStringToRemove = len( "CRInvertLepCut" )
        self.Cut = self.AllBinCuts[ "Preselection" if BinName == "CRInvertLepCutTHQLeptonicTag" else BinName[ lenStringToRemove: ] ]
        
        if not EffPlotName :
            EffPlotName = BinName[ lenStringToRemove: ]
        ...
        self.HiggsSamples["thq"] = DatasetInfo("thq" , "../../signals/14August1percentSystLimit/out_ctcv_thq_syst.root", "ctcv", "RVthq_mh125" , "2dPlots.root" ,   EffPlotName, 0.99999 , 1.00001 , self.Cut )
        self.HiggsSamples["thw"] = DatasetInfo("thw" , "../../signals/14August1percentSystLimit/out_ctcv_thw_syst.root", "ctcv", "RVthw_mh125" ,  "2dPlots.root" ,  EffPlotName, 0.99999 , 1.00001 , self.Cut )
        self.HiggsSamples["tth"] = DatasetInfo("tth" , "../../signals/14August1percentSystLimit/out_tth_syst.root", "cms_hgg_13TeV", "RVtth_mh125",  "2dPlots.root",EffPlotName , 0.99999 ,1.00001 , self.Cut )
        #self.HiggsSamples["vh"]  = DatasetInfo("vh" , "../../signals/14August1percentSystLimit/out_vh_syst.root", "cms_hgg_13TeV", "RVvh_mh125" ,  "2dPlots.root" , EffPlotName, 0.99999 , 1.00001 , self.Cut )
...
data_in_SR = 62.
data_in_CR = 140.
bkgRate_ = MC_in_SR/data_in_CR
Preselection = BinDatacard( "CRInvertLepCutTHQLeptonicTag", "Preselection" , bkgRate=bkgRate_ )
AllBins = {}
AllBins["THQLeptonic"] = {"THQ":BinDatacard( "CRInvertLepCutTHQLeptonicTHQTag" , bkgRate=bkgRate_) ,
                          "TTH":BinDatacard( "CRInvertLepCutTHQLeptonicTTHTag" , bkgRate=bkgRate_) }
#AllBins["MVA"] = { "THQ":BinDatacard("MVATHQ"),
#                   "TTH":BinDatacard("MVATTH") }
AllBins["EtaNJet"] = {"THQ":BinDatacard("CRInvertLepCutEtaNJetTHQTag" , bkgRate=bkgRate_),
                      "TTH":BinDatacard("CRInvertLepCutEtaNJetTTHTag", bkgRate=bkgRate_) }
AllBins["EtaNbJet"] = {"THQ":BinDatacard("CRInvertLepCutEtaNbJetTHQTag", bkgRate=bkgRate_),
                       "TTH":BinDatacard("CRInvertLepCutEtaNbJetTTHTag", bkgRate=bkgRate_) }
AllBins["NJetNbJet"] = {"THQ":BinDatacard("CRInvertLepCutNJetNbJetTHQTag", bkgRate=bkgRate_),
                        "TTH":BinDatacard("CRInvertLepCutNJetNbJetTTHTag", bkgRate=bkgRate_) }
...
for kvkt in AllCtOverCVs:
    ...
    fRun.write("cd FinalFit/datacards/CRInvLepCut22July/%s\n" % (dirname) )
    ...
    fRun.write("combine --X-rtd ADDNLL_RECURSIVE=0 -n Preselection  -M  Asymptotic Bin%s.root --run=blind -m 125 --ct=%g --cv=%g\n" % (Preselection.BinName, kf, kv ) )
    ...
    for Bin in AllBins:
        ...
        fRun.write("combine --X-rtd ADDNLL_RECURSIVE=0 -n %s -M Asymptotic Combined%s.root --run=blind -m 125 --ct=%g --cv=%g\n" % (Bin , Bin , kf,kv) )
