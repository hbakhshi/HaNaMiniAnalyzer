...
top_files = {
    DIR + "/bkgs/TTGG.root": ("ttgg" , TColor.GetColor("#ffcc00") , "TTGG" , "TTGG" ),
    DIR + "/bkgs/TTGJets.root":("ttgj" , kGreen+2 , "TTGJets" , "TTGJets" ),
    #DIR + "/bkgs/TGJets_TuneCUETP8M1_13TeV_amcatnlo_madspin_pythia8.root":("tgjets" , kBlue , "TGJets_TuneCUETP8M1_13TeV_amcatnlo_madspin_pythia8" , "TGJets" ),
    DIR + "/bkgs/TTJets_TuneCUETP8M1_13TeV-madgraphMLM-pythia8.root" : ("TTJet" , TColor.GetColor("#993333") , "TTJets_TuneCUETP8M1_13TeV-madgraphMLM-pythia8" , "TTJetsMG"),
    DIR + "/bkgs/TTJets_TuneCUETP8M2T4_13TeV-amcatnloFXFX-pythia8.root" : ("TTJet" , kRed , "TTJets_TuneCUETP8M2T4_13TeV-amcatnloFXFX-pythia8" , "TTJetsAMC") 
}
allSTs = [ ]
for t_file,info in top_files.items() :
    treename = TreeTemN % info[0]
    color = info[1]
    fname = info[2]
    name = info[3]
    sample_t_ff = Sample( fname + "_ff" , -1 , False , "" , treeName=treename )
    sample_ff = ExtendedSample( sample_t_ff , additionalCut = cutJetJet )
    sample_ff.LoadJobs( DIR + "/bkgs/" , fname + ".root" )
    
    sample_t_fp = Sample( fname + "_fp" , -1 , False , "" , treeName=treename )
    sample_fp = ExtendedSample( sample_t_fp , additionalCut = cutGammaGamma )
    sample_fp.LoadJobs( DIR + "/bkgs/" , fname + ".root" )
    
    sample_t_pp = Sample( fname + "_pp" , -1 , False , "" , treeName=treename)
    sample_pp = ExtendedSample( sample_t_pp , additionalCut = cutJetGamma )
    sample_pp.LoadJobs( DIR + "/bkgs/" , fname + ".root" )
    
    t_sample = SampleType( name , color ,  []  )
    t_sample.Samples.extend( [sample_pp , sample_fp , sample_ff] )
    allSTs.append( t_sample )


HiggsSamples = { DIR + "signals/THW.root" : "thw_125",
               DIR + "signals/TTH.root" : "tth_125",
               DIR + "signals/GGH.root": "ggh_125",
               DIR + "signals/VBF.root": "vbf_125",
               DIR + "signals/VH.root" : "vh_125",
               DIR + "signals/THQ.root" : "thq_125"
               }
Higgs = SampleType( "Higgs" , kOrange , [ Sample( os.path.basename(s).split('.')[0] , -1 , False , "" , treeName=TreeTemN % (t) ) for s,t in HiggsSamples.items()  ] , DIR + "/signals/" , True )
allSTs.append( Higgs )

###############  SAMPLE LIST CREATED #############


appendix = "allTopSamples_mvaonly"
...
cDiG = CutInfo( "DiG2J" , "&&".join( [ Cuts[s] for s in ["DiG","met"] ] ) , "weight" , "di#gamma + met cut ("+str(LUMI)+"fb^{-1})" )
...
allCuts = [ cDiG , cDiGNoLeptons , cDiG2J1T , cDiGLeptons2j1t , cDiGEle, cDiGMu]

for cut in allCuts :
    cut.AddHist( "nVertices" , "nvtx", 10 , 0. , 40. , "#vertices" , dirName = "PlottedVars" , MCOnly = True )
    mgg = cut.AddHist( "mGG",  "CMS_hgg_mass" , 20 , 100 , 180. , "#gamma#gamma invariant mass (GeV);#entries" , dirName = "PlottedVars", MCOnly = True)
    ggmva = cut.AddHist( "GGmva",  "diphoMVA" , 100 , -1. , 1. , "#gamma#gamma mva;#entries", dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( ggmva , mgg , MCOnly=False , dirName="PlottedVars" )
    
    cut.AddHist( "maxGmva",  "((leadIDMVA>subleadIDMVA)*leadIDMVA +  (leadIDMVA<=subleadIDMVA)*subleadIDMVA)" , 10 , -1. , 1. , "max(#gamma_{1} mva,#gamma_{2} mva);#entries", dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( "minGmva",  "((leadIDMVA>subleadIDMVA)*subleadIDMVA +  (leadIDMVA<=subleadIDMVA)*leadIDMVA)" , 5 , -1. , 1. , "min(#gamma_{1} mva,#gamma_{2} mva);#entries", dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( "mGGFineBinned",  "CMS_hgg_mass" , 160 , 100 , 180. , "#gamma#gamma invariant mass (GeV);#entries", dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( "nJets" , "n_jets" , 10 , 0. , 10. , "#Jets;#entries" , dirName = "PlottedVars", MCOnly = True)
    #cDiGLeptons.AddHist( "LepPt" , "LeptonPt",  7 , 20. , 160. , "a;Lepton p_{T} (GeV);#entries" )
    cut.AddHist( "nbJets" , "n_M_bjets" , 5 , 0. , 5., "#b-tagged jets;#entries" , dirName = "PlottedVars", MCOnly = True)
    met1 = cut.AddHist( "met" , "MET_pt",  3 , 30. , 180. , "MET (GeV);#entries" , dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( "jPt" , "jet1_pt" ,  10 , 10. , 310. , "leading jet p_{T};#entries" , dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( "jEta" , "abs(fwdJetEta_Medium)" ,  5 , 0 , 5.0 , "forward jet |#eta|;#entries" , dirName = "PlottedVars", MCOnly = True)
    leptT = cut.AddHist( "LeptonType" , "LeptonType" ,  5 , 0 , 5.0 , "Lepton Type;#entries" , dirName = "PlottedVars", MCOnly = True)
    cut.AddHist("metFinnedBins1" , "MET_pt",  100 , 0. , 500. , "MET (GeV);#entries" , dirName = "PlottedVars", MCOnly = True)
    cut.AddHist( met1 , leptT , MCOnly = True , dirName="PlottedVars")
    dipho_lead_prompt = cut.AddHist("dipholeadprompt" , "dipho_lead_prompt",  5 , -1. , 4 , "LeadingIsPrompt" , dirName = "PlottedVars", MCOnly = True)
    dipho_sulead_prompt = cut.AddHist("diphosuleadprompt" , "dipho_sulead_prompt",  5 , -1. , 4 , "SubLeadingIsPrompt" , dirName = "PlottedVars", MCOnly = True)
   
    cut.AddHist( dipho_sulead_prompt , dipho_lead_prompt , MCOnly = False , dirName="PlottedVars")

    dipho_lead_prompt_ = cut.AddHist("Isdipholeadprompt" , "dipho_lead_prompt==1",  2 , 0 , 2 , "LeadingIsPrompt" , dirName = "PlottedVars", MCOnly = True)
    dipho_sulead_prompt_ = cut.AddHist("Isdiphosuleadprompt" , "dipho_sulead_prompt==1",  2 , 0 , 2 , "SubLeadingIsPrompt" , dirName = "PlottedVars", MCOnly = True)

    cut.AddHist( dipho_sulead_prompt_ , dipho_lead_prompt_ , MCOnly = False , dirName="PlottedVars" )
...
for var_ in variablesUsed:
    ...
    if var in AutoLimits:
        ...
        #for cut in allCuts:
        #    cut.AddHist(var , var , 10 , Auto = True , Title=title, MCOnly = True)
...
plotter.Write(fout, False , plotNormalizedOfAll = True)
fout.Close()
